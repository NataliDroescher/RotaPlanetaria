...
def verificar_combustivel(caminhos, fuel_available):
    ...
    for caminho, custo in caminhos:
        ...
        for i in range(len(caminho) - 1):
            ...
            # Obter o peso entre os planetas
            peso = distances.get((origem, destino), distances.get((destino, origem), float('inf')))

            # Verificar reabastecimento (uma única vez por estação)
            if origem in estacoes_espaciais and origem not in estacoes_visitadas:
                combustivel_restante += 1000  # Reabastecimento
                estacoes_visitadas.add(origem)

            # Subtrair o peso do combustível restante
            combustivel_restante -= peso

            # Verificar se o combustível acabou durante o trajeto
            if combustivel_restante < 0:
                mensagem_caminhos.append(f"Caminho {' -> '.join(caminho)}: INVIÁVEL (Falta de combustível)")
                break
        else:
            # Se o loop não foi interrompido, o caminho é viável
            mensagem_caminhos.append(
                f"Caminho {' -> '.join(caminho)}: Viável (Combustível restante: {combustivel_restante})"
            )
            caminhos_viaveis.append((caminho, custo, combustivel_restante))  # Adiciona caminho viável com combustível restante

    # Erro se nenhum caminho for viável
    if not caminhos_viaveis:
        raise ValueError("Nenhum caminho viável devido à falta de combustível.")
    print("comb tt dentro função: ", fuel_available) 
    print("comb rest dentro função: ", combustivel_restante)   
...
def bellman_ford(adj_matrix, vertices, origem, destino):
    ...
    distancias = [float('inf')] * num_vertices
    distancias[origem_idx] = 0
    caminhos = {i: [] for i in range(num_vertices)}  # Inicializar caminhos para todos os vértices
    caminhos[origem_idx] = [[origem]]  # Caminho inicial para o vértice de origem
        ...
        for i in range(num_vertices):
            for j in range(num_vertices):
                ...
                if peso < float('inf') and distancias[i] + peso < distancias[j]:
                    ...
                    # Atualizar caminhos sem duplicados ou ciclos
                    novos_caminhos = []
                    for caminho in caminhos[i]:
                        if vertices[j] not in caminho:  # Evitar ciclos
                            novo_caminho = caminho + [vertices[j]]
                            if novo_caminho not in novos_caminhos:  # Evitar duplicados
                                novos_caminhos.append(novo_caminho)
                    caminhos[j] = novos_caminhos
                    ...
                    for caminho in caminhos[i]:
                        if vertices[j] not in caminho:  # Evitar ciclos
                            novo_caminho = caminho + [vertices[j]]
                            if novo_caminho not in caminhos[j] and novo_caminho not in novos_caminhos:  # Evitar duplicados
                                novos_caminhos.append(novo_caminho)
                    caminhos[j].extend(novos_caminhos)
    ...
    if origem and destino and algorithm:
        ...
        try:
            ...
            if caminhos_viaveis:
                ...
                print("comb rest: ", combustivel_restante)     
